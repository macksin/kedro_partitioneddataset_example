...
def make_dataset(random_seed: int = 42, sample_size: int = 10) -> pd.DataFrame:
    X, y = make_classification(
        n_samples=sample_size, n_features=20, random_state=random_seed
    )
...
def generate_big_data(
    random_seed: int = 42, n_partitions: int = 10, sample_size: int = 10
) -> Dict[str, Callable[[], Any]]:

    rng = np.random.seed(random_seed)

    return {
        f"dataset_part_{i}": lambda: make_dataset(
            random_seed=rng, sample_size=sample_size
        )
